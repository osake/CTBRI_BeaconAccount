	...
	public List<UserDTO> listResults(int start, int limit, String sortName,
		...
		for (UserDTO userDTO : userDTOList) {
			...
			if(userDTO.getType().equals("admin"))
				userDTO.setType("管理员用户");
			else if(userDTO.getType().equals("prouser"))
				userDTO.setType("省公司用户");
		}
	...
	public UserCheckForm getInfoForCheck(String userId) {
		...
		if (userInfo!=null) {
			...
			if(userInfo.getType().equals("admin"))
				userCheckForm.setType("管理员用户");
			else if(userInfo.getType().equals("prouser"))
				userCheckForm.setType("省公司用户");
			else
