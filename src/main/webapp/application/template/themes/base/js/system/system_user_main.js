	   ...
	   // multiselect: true,
	    ...
	    gridComplete:function(){  //在此事件中循环为每一行添加详细信息	     	
			...
			for(var i=0; i<ids.length; i++){
				...
				setupstr = "<img id='" + id + "' src='"+global_context+"/application/template/themes/base/images/user_modify.gif' title='修改' class='auth' authKey = 'user_mod'  style='cursor:pointer;' onclick='modifyUserIn(this.id);'/>&nbsp;&nbsp;";
				setupstr += "<img id='" + id + "' src='"+global_context+"/application/template/themes/base/images/user_check.gif' title='查看' class='auth' authKey = 'user_check' style='cursor:pointer;' onclick='checkUser(this.id);'/>&nbsp;&nbsp;";
				if(rowData.status=="正常态"){
					setupstr += "<img id='" + id + "' src='"+global_context+"/application/template/themes/base/images/reset.gif' title='恢复密码' class='auth' authKey = 'user_reset' style='cursor:pointer;' onclick='recoverUserPassIn(this.id);'/>&nbsp;&nbsp;";
					setupstr += "<img id='" + id + "' src='"+global_context+"/application/template/themes/base/images/lockUser.gif' title='冻结' class='auth' authKey = 'user_lock' style='cursor:pointer;' onclick='lockUserCF(this.id);'/>&nbsp;&nbsp;";
					setupstr += "<img id='" + id + "' src='"+global_context+"/application/template/themes/base/images/expireUser.gif' title='废弃' class='auth' authKey = 'user_expire' style='cursor:pointer;' onclick='expireUserCF(this.id);'/>&nbsp;&nbsp;";
			//		setupstr += "<img id='" + id + "' src='"+global_context+"/application/template/themes/base/images/user_del.gif' title='删除' style='cursor:pointer;' onclick='delUserCF(this.id);'/>";
					...
					setupstr += "<img id='" + id + "' src='"+global_context+"/application/template/themes/base/images/unlockUser.gif' title='解冻' class='auth' authKey = 'user_unlock' style='cursor:pointer;' onclick='unLockUserCF(this.id);'/>";
				}else if(rowData.status=="废弃态"){
					setupstr += "<img id='" + id + "' src='"+global_context+"/application/template/themes/base/images/recoverUser.gif' title='启用' class='auth' authKey = 'user_restart' style='cursor:pointer;' onclick='recoverUserCF(this.id);'/>&nbsp;&nbsp;";
			//		setupstr += "<img id='" + id + "' src='"+global_context+"/application/template/themes/base/images/user_del.gif' title='删除' style='cursor:pointer;' onclick='delUserCF(this.id);'/>";
				}
