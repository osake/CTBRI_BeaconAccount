...
function addHoverDom(treeId, treeNode) {
	...
	if(treeNode.status!=null){		
		var str =  "<div id='fadeIn' style='margin:10px 10px;display:none;'><h3>公司："+treeNode.name+"</h3>" 
			                  +"<h3>状态："+treeNode.status+"</h3>"
								+"<h3>版本："+treeNode.dataVersion+"</h3>"
								   +"<h3>容量："+treeNode.capacity+"</h3></div>";
		$("#content_body").html(str);
		$("#fadeIn").fadeIn(1000);
	}
...
function modifyOrganIn(treeNode) {
	...
	$("#content_body").load(global_jsp_position+'/system/system_organ_modify_dialog.jsp',function(data){
		$("#modify_dep").val(treeNode.departmentId);
		$("#modify_dep_info").val(treeNode.departmentInfoId);
		$("#modify_name").val(treeNode.name);
		$("#modify_status").val(treeNode.status);
		$("#modify_version").val(treeNode.dataVersion);
		$("#modify_capacity").val(treeNode.capacity);
...
function modifyOrganInCB(treeNode) {
	...
	$("#modifyOrganForm").validate({
		rules:{
			name:{required:true,maxlength:50,byteRangeLength:[0,50],treeNodeNameExisted:["organ_tree",treeNode,name]}
		},
		onkeyup:false,
		errorPlacement: function(error, element) { //指定错误信息位置
			//element.parent().children().filter("span").append("<br/>");
...
function modifyOrganOutCB(data){
	...
	if(data.result=="success") {
		...
		currentNode.status = data.identity.status;
		currentNode.dataVersion = data.identity.dataVersion;
		currentNode.capacity = data.identity.capacity;
